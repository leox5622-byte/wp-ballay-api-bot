...
const { configCommands } = global.DoraBot;
...
module.exports = {
	...
	onStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, event, commandName, getLang }) => {
		...
		if (
			...
			if (url.match(/(https?:\/\/(?:www\.|(?!www)))/)) {
				...
				const messageBody = event.message?.conversation || event.message?.extendedTextMessage?.text || event.body || "";
				if (args[args.length - 1].endsWith(".js")) {
					fileName = args[args.length - 1];
					rawCode = messageBody.slice(messageBody.indexOf('install') + 7, messageBody.indexOf(fileName) - 1);
				}
				else if (args[1].endsWith(".js")) {
					fileName = args[1];
					rawCode = messageBody.slice(messageBody.indexOf(fileName) + fileName.length + 1);
				}
				else
					return message.reply(getLang("missingFileNameInstall"));
			}
			...
			if (fs.existsSync(path.join(__dirname, fileName)))
				...
				global.DoraBot.onReaction.set(info.messageID, {
					commandName,
					messageID: info.messageID,
					type: "install",
					author: event.key?.participant || event.senderID || event.author || event.from,
					data: {
						fileName,
						rawCode
					}
	...
	onReaction: async function ({ Reaction, message, event, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang }) {
		...
		const userId = event.key?.participant || event.userID || event.author || event.from;
		if (userId != author)
			return;
		const infoLoad = loadScripts("cmds", fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode);
		infoLoad.status == "success" ?
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
	// global.DoraBot[folderModules == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
		// 	filePath: pathCommand,
		// 	commandName: [commandName, ...validAliases]
		// });
		const storageCommandFilesPath = global.DoraBot[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];

	try {
		if (rawCode) {
			fileName = fileName.slice(0, -3);
			fs.writeFileSync(path.normalize(`${process.cwd()}/commands/${fileName}.js`), rawCode);
		}
		const regExpCheckPackage = /require(\s+|)\((\s+|)[`'"]([^`'"]+)[`'"](\s+|)\)/g;
		const { DoraBot } = global;
		const { onFirstChat: allOnFirstChat, onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = DoraBot;
		...
		if (process.env.NODE_ENV == "development") {
			const devPath = path.normalize(process.cwd() + `/commands/${fileName}.dev.js`);
			if (fs.existsSync(devPath))
				pathCommand = devPath;
			else
				pathCommand = path.normalize(process.cwd() + `/commands/${fileName}.js`);
		}
		else
			pathCommand = path.normalize(process.cwd() + `/commands/${fileName}.js`);
		...
		// —————————————— CHECK COMMAND EXIST ——————————————— //
		if (!oldCommandName) {
			if (DoraBot[setMap].get(oldCommandName)?.location != pathCommand)
			throw new Error(`${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(DoraBot[setMap].get(oldCommandName)?.location || "")}"`);
		}
		// ————————————————— CHECK ALIASES ————————————————— //
		...
		if (configCommand.aliases) {
			...
			for (const alias of aliases) {
				...
				if (DoraBot.aliases.has(alias))
					throw new Error(`alias "${alias}" is already exist in ${commandType} "${DoraBot.aliases.get(alias)}" with file name "${removeHomeDir(DoraBot[setMap].get(DoraBot.aliases.get(alias))?.location || "")}"`);
				DoraBot.aliases.set(alias, scriptName);
			}
		}
		// ————————————————— CHECK ENVCONFIG ————————————————— //
		// env Global
		...
		DoraBot[setMap].delete(oldCommandName);
		DoraBot[setMap].set(scriptName, command);
...
function unloadScripts(folder, fileName, configCommands, getLang) {
	const pathCommand = `${process.cwd()}/commands/${fileName}.js`;
	...
	const { DoraBot } = global;
	const { onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = DoraBot;
	...
	delete require.cache[require.resolve(pathCommand)];
	DoraBot[setMap].delete(commandName);
